...
Every message you send to the user starts with the line `[hourly]` on its own,
before anything else, so the user can tell which project is talking when several
Claude Code sessions run in parallel.

`hourly` is a **working directory name, not a name**. The project's real name is
the name of the species the collection depicts — the way `Quantums` names a
species and not a mechanism — and it is not chosen yet. Until the owner picks
it, nothing in this repository may read as if the name were final: no wordmark
baked into an image, no name inside a database value, no name in a migration, no
ticker in a constant. When it lands it touches exactly three places:
