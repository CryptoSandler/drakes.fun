...
Run all of them, in this order, and paste each one's full output. The Rust suite is not
optional in a batch that touched `programs/`: the program and the TypeScript perform the
same arithmetic and the vectors are what keep them honest.

```bash
EV=~/proyectos/evidencia/drakes/$(date -u +%Y-%m-%d)-$(git branch --show-current); mkdir -p "$EV"

npm run typecheck > "$EV/typecheck.log" 2>&1; echo "TYPES EXIT: $?"
npm run lint      > "$EV/lint.log"      2>&1; echo "LINT  EXIT: $?"
npm test          > "$EV/test.log"      2>&1; echo "TEST  EXIT: $?"
npm run build     > "$EV/build.log"     2>&1; echo "BUILD EXIT: $?"
cargo test --manifest-path programs/issuance/Cargo.toml > "$EV/cargo-test.log" 2>&1; echo "RUST  EXIT: $?"
```
...
`npm test` needs `TEST_DATABASE_URL` set and the test database reachable —
`vitest.global-setup.ts` asserts the `disposable_database` stamp before any test file is
allowed to touch anything, so a missing or wrong test database aborts the run rather than
quietly truncating the wrong one. If it aborts, that is a blocked close (see rule 2). Do
...
**A batch that adds a file to `migrations/` is not closed until test, preview and
production all report it — in that order.** The suite cannot see this: it runs against
...
migrated. Every other one is invisible to every other gate.
    ...
    npm run migrate:test    > "$EV/migrate-test.log" 2>&1;    echo "EXIT: $?"
    npm run migrate:preview > "$EV/migrate-preview.log" 2>&1; echo "EXIT: $?"
    npm run migrate         > "$EV/migrate-prod.log" 2>&1;    echo "EXIT: $?"

Then **ask each database what it holds**, rather than trusting that the commands ran —
"applied 0" is what an up-to-date database and a misconfigured one both print:

    npm run check:migrations > "$EV/check-migrations.log" 2>&1; echo "EXIT: $?"

It prints one line per database with its **host** and its full version list, and exits
non-zero when they disagree (`1`), when fewer than three are configured (`2`), or when the
`disposable_database` stamp is anywhere but `tests` (`3`). **A disagreement is a blocked
close, not a note in the report.**

It compares the whole version list rather than `max(version)`, because two databases can
share a highest version and disagree about a gap underneath it, and a `max()` check calls
that agreement.

**Measured 2026-09-01 in this repository.** Migration `0004` added
`hoard_purchases.funded_by`, was applied to the test database by the test run, and never to
production. Nothing failed. The column simply did not exist, so the row came back without
the field, and `/verify` rendered `funded_by === 'creator' ? … : 'fees'` — **printing
`fees` for a row whose provenance the database did not record**, on a public page, in the
one column of that table that is an assertion rather than a derivation. The same check then
found the preview database had **never been migrated at all**. `~/.claude/GATES.md` has the
incident and the sibling one in `drakes`.

**The second half of that lesson is in the code, not here:** a value outside the two the
schema admits — `undefined` included — is rendered as a fault and never as either of them
(`src/lib/site/provenance.ts`). A ternary has two branches and there are three cases.

### The noindex triple, checked against the domain and not against intent

`drakes.fun` is live, public and **reading devnet** (D29), and a push to `main` deploys
production. So a close that touched the site checks the three independent noindex legs on
the **domain**, with deployment protection off — a 401 from a protected deployment answers
every question with the same "no", including this one:

    curl -sI https://drakes.fun/ | grep -i x-robots-tag
    curl -s  https://drakes.fun/ | grep -o '<meta name="robots" content="[^"]*"'
    curl -s  https://drakes.fun/robots.txt

Three, because any one of them can be lost in a config change and none of them reports its
own absence. And a **control**: a route that does not exist must answer `404`, or the
`200`s above are a catch-all rather than the pages.

**And the rehearsal disclosure is checked with them.** While the chain is devnet, the chip
must render filled and the sentence must be present; both are conditioned on the
server-side cluster classification, so the day the env points at mainnet they disappear on
their own and this check goes with them.
...
| Claim | Evidence |
|---|---|
| e.g. A deposit from the wrong wallet does not publish a raffle | `escrow.test.ts` — 4 passing cases, output in §2 |
...
command agrees with itself. Measured for this repository on 2026-09-02:
    ...
    SKIPS   da9cb87 (docs/ only)
    BUILDS  a6b1592 (app/, src/)
